...
basic_infov3 <- rename(basic_info, Height = Length)

basic_infov2 <- post_meal_data %>%
...
tidied_post_meal_data <- tidier_post_meal_data %>%
  mutate(
    insuline_glucose_ratio = auc_insuline / auc_glucose,
...
tidier_post_meal_data %>%
  mutate(insuline_glucose_ratio = auc_insuline / auc_glucose)
...
tidied_post_meal_data %>%
  ...
  ggplot(aes(x = BMI, y = insuline_glucose_ratio, colour = Group)) +
  geom_point() +
...
```{r}
tidied_post_meal_data %>% 
  summarise(max_bmi=max(BMI),min_BMI=min(BMI),mean_BMI=mean(BMI),sd(BMI))


```


## Summarize with gorup by
```{r}
tidied_post_meal_data %>% 
  group_by(Group) %>% 
